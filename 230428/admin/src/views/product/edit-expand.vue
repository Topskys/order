        ...
        <el-table-column label="PDF" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-form-item style="overflow: hidden;">
              ...
              <!--              </el-upload>-->
              <a :href="scope.row.document_url" v-if="scope.row.document_url">{{ scope.row.document_url }}</a>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        tableData: [
// { title: '1', icon: '2', document_url: '', showSave: false }
        ]
      },
      currentRow: '',
      currentFile: ''
  ...
  methods: {
    ...
    add() {
      ...
      if(!prevIsSave) {
        showWarn('上条记录未保存，请先保存')
        return
      }
      arr.length >= 4?showWarn('客官，最多只能添加4条'):this.form.tableData.push(obj)
    },
    // 保存按钮
    async save(row) {
        if (this.currentFile && row.title) {
          const formData = new FormData()
          formData.append('file', this.currentFile)
          const {url}=await this.$upload.uploadFile(formData)
          row.document_url = url

          const { msg } = await this.$product.addExpand({product_id:this.productId,...row})
          this.$message({
            type: 'success',
            message: msg
          })
